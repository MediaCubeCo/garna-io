...
### Testimonial quote and author text

Use this for testimonial cards such as AI Hiring / `What businesses say`.

Approved style:

- Quote text: Manrope, `text-base` / `16px`, `font-normal` / `400`, `leading-relaxed` / roughly `26px`, gray-300 `#D1D5DB`, with `mb-8` / `32px` before the author row.
- Author name: Manrope, `text-sm` / `14px`, `font-normal` / `400`, white `#ffffff`.
- Author role/company: Manrope, `text-xs` / `12px`, `font-medium` / `500`, primary green `#5EA500`.

Rules:

- Use this typography only for testimonial/review cards.
- Keep testimonial quote text lighter than card titles and brighter than normal muted card descriptions.
- Do not add quote marks in the catalog example; page copy can keep real sentence punctuation if needed.
...
This is the shared base for approved content cards. Right now the approved card types are Contractor benefit card and Testimonial card.
...
- Top row: five small yellow `iconify-icon` stars using `solar:star-bold`, `14px`, `text-yellow-500`, `gap-0.5`; no quote icon.
