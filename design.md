...
This is the shared base for approved content cards. Right now the approved card types are Contractor benefit card, Testimonial card, Talent job card, Feature visual card, Visual content card and White Label model card.
...
- Desktop: `grid-cols-2`, `gap-8`, roughly `32px`.
- Mobile: one column.
...
- Keep tab labels compact and avoid card-like pills unless a separate tab variant is approved.
