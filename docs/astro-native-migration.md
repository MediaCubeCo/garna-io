...
- `set:html` is restricted to `components/sections/blog/TrustedDynamicHead.astro`, where the Worker replaces the documented blog metadata marker with trusted CMS meta and JSON-LD. It is not a page-composition mechanism.
...
## Final directory boundaries

- `astro/pages` contains route entrypoints only.
- `astro/layouts` contains `BaseLayout` and reusable full-page compositions.
- `astro/components/sections/<family>` contains domain-specific sections and their colocated interactions.
- `astro/components/{ui,layout,visuals}` contains reusable primitives, structural wrappers, and HTML-rendered product graphics.
- `astro/components/pages` and `astro/components/dynamic` are forbidden intermediate categories.
